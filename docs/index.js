...
var inverseLaplaceCamulative = function inverseLaplaceCamulative(u) {
	return -sign(u - 0.5) * Math.log(1 - 2 * Math.abs(u - 0.5));
};
var laplaceDistribution = function laplaceDistribution() {
	var uniformRandom = Math.random();
	var laplaceRandom = inverseLaplaceCamulative(uniformRandom);
	return laplaceRandom;
};

function setupLaplaceGraph(svg, height) {
	var rects = svg.selectAll('rect').data(sampledData).enter().append('rect').attr('x', function (d, i) {
		return i * 30;
	}).attr('y', function (d) {
		return height - d;
	}).attr('height', function (d) {
		return d;
	}).attr('width', 15).attr('fill', 'lightgreen');
	var axises = svg.selectAll('text').data(sampledData).enter().append('text').attr("text-anchor", "middle").attr('x', function (d, i) {
		return i * 30 + 8;
	}).attr('y', function (d) {
		return height - 10;
	}).text(function (d, i) {
...
var dischargedNumber = 0;
var presentDistribution = laplaceDistribution;
var sampledData = Array.apply(null, new Array(21)).map(function () {
	return 0;
});
...
var addOneRandom = function addOneRandom() {
	var randomNumber = presentDistribution();
...
$(document).ready(function () {
	...
	var svg = d3.select('#svg').style('background-color', '#223344');
	var update = function update() {
		var heightScaler = d3.scaleLinear().domain([0, Math.max(d3.max(sampledData, function (d) {
			return d;
		}), height - 10)]).range([0, height - 10]);
		var rects = svg.selectAll('rect').data(sampledData).transition().duration(500).attr('y', function (d) {
			return height - heightScaler(d);
		}).attr('height', function (d) {
			return heightScaler(d);
		});
		$('#discharged').text("discharged: " + dischargedNumber);
		$('#sampled').text("sampled: " + sampledData.reduce(function (prev, curr) {
			return prev + curr;
		}, 0));
	};
