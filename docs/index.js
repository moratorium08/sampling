...
function setupLaplaceGraph(svg, height) {
	const rects = svg.selectAll('rect').data(sampledData).enter().append('rect').attr('x', (d, i) => {
		return i * 30;
	}).attr('y', d => {
		return height - d;
	}).attr('height', d => {
	...
	const axises = svg.selectAll('text').data(sampledData).enter().append('text').attr("text-anchor", "middle").attr('x', (d, i) => {
		return i * 30 + 8;
	}).attr('y', d => {
		return height - 10;
	}).text((d, i) => {
...
const sampledData = Array.apply(null, new Array(21)).map(() => {
	return 0;
});

const reset = () => {
	for (let i = 0; i < 21; i++) sampledData[i] = 0;
	dischargedNumber = 0;
};
...
$(document).ready(() => {
	...
	const update = () => {
		const heightScaler = d3.scaleLinear().domain([0, Math.max(d3.max(sampledData, d => {
			return d;
		}), height - 10)]).range([0, height - 10]);
		const rects = svg.selectAll('rect').data(sampledData).transition().duration(500).attr('y', d => {
	...
	$('#add-ten').click(() => {
		for (let i = 0; i < 10; i++) addOneRandom();
		update();
	});
